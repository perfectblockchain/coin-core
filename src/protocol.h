...
#ifndef RIZCOIN_PROTOCOL_H
#define RIZCOIN_PROTOCOL_H
...
enum ServiceFlags : uint64_t {
    ...
    // set by all Rizcoin Core nodes, and is unset by SPV clients or other peers that just want
    // network services but don't provide them.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo protocol request.
    // Rizcoin Core does not support this but a patch set called Bitcoin XT does.
    // See BIP 64 for details on how this is implemented.
    NODE_GETUTXO = (1 << 1),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered connections.
    // Rizcoin Core nodes used to support this by default, without advertising this bit,
