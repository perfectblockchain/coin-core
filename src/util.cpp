...
const char * const RIZCOIN_CONF_FILENAME = "rizcoin.conf";
const char * const RIZCOIN_PID_FILENAME = "rizcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "rizcoin" is a composite category enabling all Rizcoin-related debug output
            if(ptrCategory->count(string("rizcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\rizcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\rizcoinCore
    // Mac: ~/Library/Application Support/rizcoinCore
    // Unix: ~/.rizcoinCore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "rizcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/rizcoinCore";
#else
    // Unix
    return pathRet / ".rizcoinCore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", RIZCOIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty rizcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override rizcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", RIZCOIN_PID_FILENAME));
